...
function showCards() {
  ...
  imgPosts.forEach((post) => {
    clutter += `<div class="box">
        <img class="cursor-pointer" src="${post.image}" alt="image">
...
function searhingPosts() {
  ...
  document.querySelector("#searchinput").addEventListener("input", function (inp) {
      var searchData = document.querySelector(".searchdata");
      if (inp.target.value !== "") {
        var filterArr = imgPosts.filter(function (obj) {
          return obj.name.toLowerCase().startsWith(inp.target.value);
        });
